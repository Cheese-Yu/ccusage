...
import { getLastDaysRange, normalizeLastDays } from '../date-utils.ts';
...
export const lastCommand = define({
	...
	args: {
		...
		day: {
			type: 'string',
			description: 'Number of recent days to include (must be a positive integer, default: 7)',
			default: '7',
		},
	},
	async run(ctx) {
		...
		let dayCount: number;
		try {
			dayCount = normalizeLastDays(ctx.values.day);
		} catch (error) {
			logger.error(String(error));
			process.exit(1);
		}

		const { since, until } = getLastDaysRange(dayCount, ctx.values.timezone);
		const dateRangeMessage = `Date range: ${since} to ${until} (excluding today)`;
		...
		if (events.length === 0) {
			log(
				jsonOutput
					? JSON.stringify({ daily: [], totals: null, since, until })
		...
		try {
			...
			if (rows.length === 0) {
				log(
					jsonOutput
						? JSON.stringify({ daily: [], totals: null, since, until })
						: `${dateRangeMessage}\nNo Codex usage data found for the last ${dayCount} days (excluding today).`,
				);
				return;
			}
			...
			logger.box(
				`Codex Token Usage Report - Last ${dayCount} Days (excluding today, Timezone: ${ctx.values.timezone ?? DEFAULT_TIMEZONE})`,
			);
			logger.info(dateRangeMessage);
