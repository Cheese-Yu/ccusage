...
import { getLastNDaysDateRange } from '../date-utils.ts';
...
export const lastCommand = define({
	...
	args: {
		...
		day: {
			type: 'number',
			description: 'Number of recent days to include (excluding today)',
			default: 7,
		},
	},
	async run(ctx) {
		...
		let until: string;
		try {
			const range = getLastNDaysDateRange(ctx.values.day, ctx.values.timezone);
			since = range.since;
			until = range.until;
		} catch (error) {
		...
		try {
					...
					JSON.stringify(
						{
							daily: rows,
							totals,
							range: { since, until, day: ctx.values.day },
						},
						null,
						2,
					),
			...
			logger.box(
				`Codex Token Usage Report - Last ${ctx.values.day} Days (excluding today, Timezone: ${ctx.values.timezone ?? DEFAULT_TIMEZONE})`,
			);
